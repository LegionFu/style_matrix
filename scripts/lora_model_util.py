...
from modules import shared, sd_models, hashes, paths, extensions
from scripts import safetensors_hack, lora_model_util, util
...
lora_models_dirs = []
lora_models_dirs.append(os.path.join(paths.models_path, "Lora"))
os.makedirs(lora_models_dirs[0], exist_ok=True)

for ext in extensions.extensions:
  ext_path = os.path.join(ext.path,"models/lora")
  if(os.path.exists(ext_path)):
    lora_models_dirs.append(ext_path)
...
def get_model_rating(filename):
  if not lora_model_util.is_safetensors(filename):
    return 0
...
def hash_model_file(finfo):
  ...
  if name != "None":
    ...
    if cached is None or stat.st_mtime != cached["mtime"]:
      if metadata is None and lora_model_util.is_safetensors(filename):
        try:
          metadata = safetensors_hack.read_metadata(filename)
